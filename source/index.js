...
const RouteSettingsSchema = Joi.object()
  .keys({
 ...
 */
const forEachRoute = (server, iterator) => {
 ...
 */
const wrapHandler =
  ({ shouldApplyHandler, handler }, origHandler) =>
  (request, reply) => {
  ...
  register(server, options, next) {
    ...
    server.ext("onPreStart", (server, next) => {
      forEachRoute(server, (route) => {
        ...
        const routeSettings = Joi.attempt(
          plugins["hapi-demo"],
          RouteSettingsSchema
        );
        route.settings.handler = wrapHandler(
          routeSettings,
